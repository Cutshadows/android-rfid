    ...
    private void read() {
        ...
        if (cb_filter.isChecked())//  过滤
        {
            if(etPtr_filter.getText().toString()==null || etPtr_filter.getText().toString().isEmpty()){
                UIHelper.ToastMessage(mContext, "La dirección de inicio de los datos filtrados no puede estar vacía.");
                return;
            }
            if(etData_filter.getText().toString()==null || etData_filter.getText().toString().isEmpty()){
                UIHelper.ToastMessage(mContext, "Los datos del filtro no pueden estar vacíos");
                return;
            }
